#!/usr/bin/env python3
"""Exercise every registered tool against real public endpoints and report what is broken.

Unit tests verify logic against fixtures; this verifies that the tools users
actually call still answer with real data from the outside world — a dead
`dig`/`ping`/`mtr`/`whois` binary, a broken TLS trust store, or a network that
blocks outbound ICMP all pass unit tests fine and fail here.

Read-only by construction: every tool in this server is a read-only
diagnostic, so unlike other servers in this family there is nothing to skip
for blast-radius reasons. See smoke_probes.py for the target endpoints
(all public, stable, chosen to need no configuration).

Usage:
    # In-process: imports the server directly.
...
async def main_async(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        # No probe here uses a date token, but the engine's signature needs
        # one regardless — kept for parity with the other servers sharing
        # this harness rather than forking the function for this one repo.
        ...
        results = await run_probes(
            ...
            # Every probe target is a public, non-sensitive endpoint (see
            # smoke_probes.py) -- unlike a device-fleet server, there is no
            # configuration or credential payload in a failure detail here.
...
def main() -> None:
    ...
    parser.add_argument("--date", default="", help="override the reference date (unused by any probe here)")
